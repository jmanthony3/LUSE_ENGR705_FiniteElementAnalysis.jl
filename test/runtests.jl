...
@testset "FiniteElementAnalysis.jl" begin
    ...
    elements = prepareelements_line(elements, A, E, L)
    U = solve(elements, nodeboundaryconditions, F_applied).U
    @test round.(U, digits=6) == [0.0, 0.0, -0.00125, 0.0]
    ## 3.2
    ...
    elements = prepareelements_line(elements, A, E, L)
    U = solve(elements, nodeboundaryconditions, F_applied).U
    @test round.(U, digits=6) == [0.0, 0.012440, 0.025]
    ## 3.3
    A, E, L = 1, 10e6, 100          # [in², psi, in]
    angles = float.([120, 0, 210])  # [°]
    L = L ./ abs.(cosd.(angles))
    elements = Dict("1"=>(1, 2), "2"=>(1, 3), "3"=>(1, 4))
    nodeboundaryconditions = [2, 3, 4]
    F_applied = [(1, (1e3, 1e3))]   # (node, (force_x [lb], force_y [lb]))
    elements = prepareelements_line(elements, A, E, L, angles, dims=2)
    sol = solve(elements, nodeboundaryconditions, F_applied, angles, dims=2)
    @test round.(sol.F, digits=6) == [1000.0, 1000.0, -851.456855, 1474.766533, 4137.878227, 0.0, -4286.421372, -2474.766533]
    @test round.(sol.U, digits=6) == [0.0413788, 0.014058, 0, 0, 0, 0, 0, 0]
    σ = axialstresses(elements, sol.U, dims=2)
    @test round.(σ, digits=6) == [1702.913710, 4137.878227, 4949.533066]
    ...
    elements = prepareelements_line(elements, A, E, L, angles, dims=2)
    U = solve(elements, nodeboundaryconditions, F_applied, angles, dims=2).U
    ...
    σ = axialstresses(elements, U, dims=2)
    ...
    elements = prepareelements_beam(elements, E, I, L)
    U = solve(elements, nodeboundaryconditions, F_applied).U
    @test round.(U, digits=6) == [0, 0, -0.05, -0.0013725490196078453, -0.14117647058823554, -0.0015686274509803966]

    # # ENGR 705-001: Finite Element Analysis - FEA 2
    # ## FEA2.1
    # E, c, t_w = 200e9, 176.5e-3, 6.48e-3    # [Pa, m, m]
    # I = 102e6 * (1/1e3)^4                   # [m⁴]
    # Q = 323473.474 * (1/1e3)^3              # [m³]
    # numberofelements = 3(10)
    # L = 18/numberofelements                 # [m]
    # x = 0:L:18
    # W(x) = -10.e3(x)                        # [N/m]
    # F_applied = [(W, 10, 20, 30)]           # (force(x), nodes...)
    # elements = Dict{String, Tuple{Int, Int}}()
    # nodeboundaryconditions = []
    # for node ∈ 1:1:numberofelements+1
    #     if node == 1
    ...
    # end
    # elements = prepareelements_beam(elements, E, I, L)
    # sol = solve(elements, nodeboundaryconditions, F_applied)
    # V = [sol.F[2i - 1] for i ∈ 1:1:length(sol.F)÷2]
    # M = [sol.F[2i] for i ∈ 1:1:length(sol.F)÷2]
    # U_y = [sol.U[2i - 1] for i ∈ 1:1:length(sol.U)÷2]
    # U_φ = [sol.U[2i] for i ∈ 1:1:length(sol.U)÷2]
    # σ = axialstresses(elements, M, c)
    # τ = shearstresses(elements, V, Q, t_w)
end
